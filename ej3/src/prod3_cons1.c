...
int remove_item(int q, int *prioridad_extraida);
...
void *hilo_consumidor(void *arg) {
  ...
  for (int i = 0; i < total_items; i++) {
    ...
    items_totales_disponibles--; // descontamos el item que vamos a procesar
    pthread_mutex_unlock(&mutc);
    ...
    while (!extraido) {
      ...
      for (int q = 0; q < NUM_PROD; q++) {
        if (pthread_mutex_trylock(&mutp[q]) == 0) {
          ...
          if (comp[q].tam > 0) {
            item = remove_item(q, &prioridad_extraida);
            extraido = 1;
            pthread_cond_signal(
...
void insert_item(int q, int n, int prioridad) {
  ...
  printf("[%lld] [PROD %d] Insertado '%d' (Prioridad %d). Longitud: %d\n",
         get_timestamp_ms(), prioridad, n, prioridad, comp[q].tam);
}
...
int remove_item(int q, int *prioridad_extraida) {
  // al ser 3 colas independientes, ya no hace falta buscar la prioridad
  // simplemente extraemos el primero (índice 0) en orden FIFO puro.
